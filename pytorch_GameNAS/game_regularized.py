GameNAS#from __future__ import unicode_literals, print_function, division
import os
import sys
import time
import math
...
from model_final import NetworkCIFAR as Final_Network
...
class Encoder(nn.Module):
    ...
    def initHidden(self):
        return torch.zeros(self.num_layer, self.batch_size, self.hidden_size).cuda()
...
class AttnDecoderRNN(nn.Module):
    ...
    def forward(self, input, hidden, encoder_outputs):
        ...
        attn_applied = torch.bmm(attn_weights.unsqueeze(1),
                                 encoder_outputs.view(self.batch_size,self.max_length,-1)).view(self.batch_size,-1)
...
class Game:
    def __init__(self,
        ...
        self.encoder_optimizer =  optim.Adam(self.encoder.parameters(),lr=args.player_learning_rate,weight_decay=args.player_weight_decay)
    ...
    def sample(self,selected_player_list,encoder_input = None):
        ...
        def generate_player_old_arcs(encoder_input,current_player_choices, player_id):
        ...
        def generate_old_arcs(current_player_choices):
            arcs_normal = []
            arcs_reduce = []
        ...
        for p_id in selected_player_list:
            p_id_arcs_normal,p_id_arcs_reduce = generate_player_old_arcs(encoder_input,self.curr_player_choices,p_id)

            self.old_normal_arcs = self.old_normal_arcs + p_id_arcs_normal
            self.old_reduce_arcs = self.old_reduce_arcs + p_id_arcs_reduce

        arcs_normal,arcs_reduce = generate_old_arcs(self.curr_player_choices)
    ...
    def compute_reward(self,current_player_choices,selected_player_list,epoch=0):
        ...
        logging.info("original {} arcs!".format(len(final_selected_arcs_reduce)))
        multi_obj_selected_arcs_normal = []
        multi_obj_selected_arcs_reduce = []

        dict_final_multi_index = {}

        num_multi_arcs=0
        for index in range(len(final_selected_arcs_reduce)):
            tmp_arch = self.MODEL.model.genotype(final_selected_arcs_normal[index], final_selected_arcs_reduce[index])
            tmp_model = Final_Network(36, 10, 20, True, tmp_arch)
            if utils.count_parameters_in_MB(tmp_model) <= self.args.max_arc_size:
                multi_obj_selected_arcs_normal.append(final_selected_arcs_normal[index])
                multi_obj_selected_arcs_reduce.append(final_selected_arcs_reduce[index])
                dict_final_multi_index[index] = num_multi_arcs
                num_multi_arcs += 1
            else:
                dict_final_multi_index[index] = -1



        for _ in range(1):

            self.MODEL.train(multi_obj_selected_arcs_normal,multi_obj_selected_arcs_reduce,epoch)

            epoch += 1

        logging.info("start valid {} arcs!".format(len(multi_obj_selected_arcs_reduce)))

        valid_acc_list = self.MODEL.infer(multi_obj_selected_arcs_normal,multi_obj_selected_arcs_reduce)

        mean_valid_acc_list = mean(valid_acc_list)
        min_valid_acc_list = min(valid_acc_list)
        min_valid_acc_list = min_valid_acc_list *0.9
        multi_obj_valid_acc_list = valid_acc_list.copy()
        multi_obj_valid_acc_list.append(min_valid_acc_list)
            ...
            for ops in range(self.num_ops):
                for batch_id in range(self.input_batch_size):
                    if self.old_normal_arcs[old_arc_index] != None:
                        ...
                        player_old_reward_list[player_id][ops][batch_id][0] = multi_obj_valid_acc_list[dict_final_multi_index[valid_index]] - mean_valid_acc_list


                        valid_index += 1

                        player_old_reward_list[player_id][ops][batch_id][1] = multi_obj_valid_acc_list[
                                                                                  dict_final_multi_index[valid_index]] - mean_valid_acc_list
        ...
        for batch_id in range(self.input_batch_size):
            for arc_class_id in range(self.num_arc_class-1):
                old_arcs_reward_list[batch_id][arc_class_id] = multi_obj_valid_acc_list[dict_final_multi_index[valid_index]] - mean_valid_acc_list
        ...
        for player_id in selected_player_list:
            ...
            for ops in range(self.num_ops):
                ...
                for batch_id in range(self.input_batch_size):
                    ...
                    if player_old_reward_list[player_id][ops][batch_id][0] == min_valid_acc_list-mean_valid_acc_list:
                        penalty = player_old_reward_list[player_id][ops][batch_id][0] - 0
                        special_penalty += 1
                    else:
                        penalty = player_old_reward_list[player_id][ops][batch_id][0] - \
                              player_old_reward_list[player_id][ops][batch_id][1]



                    for arc_class_id in range(self.num_arc_class - 1):
                        reward[batch_id][arc_class_id] = multi_obj_valid_acc_list[dict_final_multi_index[valid_index]] - mean_valid_acc_list
                        if dict_final_multi_index[valid_index] == -1:
                            special_index += 1
                            reward[batch_id][arc_class_id] = min_valid_acc_list-mean_valid_acc_list
                        else:
                            reward[batch_id][arc_class_id] = (reward[batch_id][arc_class_id]-old_arcs_reward_list[batch_id][arc_class_id]) - penalty
                        valid_index += 1


                    reward[batch_id][self.num_arc_class-1] = 0 - penalty
        ...
        multi_obj_selected_arcs_normal = [multi_obj_selected_arcs_normal[r] for r in rank_list]
        multi_obj_selected_arcs_reduce = [multi_obj_selected_arcs_reduce[r] for r in rank_list]



        return valid_acc_list,multi_obj_selected_arcs_normal,multi_obj_selected_arcs_reduce,players_reward_list
    ...
    def train_search(self):
        ...
        with torch.autograd.set_detect_anomaly(True):
            for iter_id in range(self.args.num_iterations):
                ...
                player_choices = torch.sum(player_choices,dim = -1)
